...
void citire()
{
    ...
    unsigned int nMAX;
    if((fis=fopen("produse.dat","r"))==NULL)return;	// iesire din functie in caz cu nu exista baza de date
    // deoarece in fisier exista numarul de elemente
    // se foloseste o structura repetitiva care citeste nMAX numere,nMAX fiind numarul total de numere
    fread(&nMAX,sizeof(unsigned int),1,fis);
    for(nNume=0;nNume<nMAX;nNume++){
