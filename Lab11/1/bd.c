...
void adauga_ascendent(const char *nume,float salariu)
{
    Persoana *pred=NULL;
    Persoana *crt;

    for(crt=bd_nou;crt;pred=crt,crt=crt->urm){
    ...
    if(pred){
        ...
        bd_nou=nou(nume,salariu,crt);
        }
}

Persoana *asc(){

    Persoana *bd_nou=NULL;
    Persoana *crt;

    bd_nou=nou(bd->nume,bd->salariu,NULL);

    for(crt=bd;crt;crt=crt->urm){

        adauga_ascendent(crt->nume,crt->salariu);
    }
